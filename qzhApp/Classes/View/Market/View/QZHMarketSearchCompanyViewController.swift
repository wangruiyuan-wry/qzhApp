//
//  QZHMarketSearchCompanyViewController.swift
//  qzhApp
//
...
class QZHMarketSearchCompanyViewController: QZHBaseViewController {
    // 历史搜索列表
...
// MARK: - 页面 UI设置
extension QZHMarketSearchCompanyViewController{
    ...
    func setupNavTitle(){
        ...
        navItem.rightBarButtonItem = UIBarButtonItem(title: "搜企业", img: "", target: self, action: #selector(search))
...
extension QZHMarketSearchCompanyViewController{
    ...
    func clanerHistory(){
        var _array:[String] = []
        var _cache:Dictionary<String,AnyObject> = CacheFunc().getCahceData(fileName: "searchMarketCompanyHistoryText.plist", folderName: "Market")
        _cache.updateValue(_array as AnyObject, forKey: "history")
        CacheFunc().setCahceData(fileName: "searchMarketCompanyHistoryText.plist", folderName: "Market", cacheDatas: _cache as NSDictionary)
        setupLabelItem()
    }
    ...
    func search(){
        ...
        _cache.updateValue(_array as AnyObject, forKey: "history")
        CacheFunc().setCahceData(fileName: "searchMarketCompanyHistoryText.plist", folderName: "Market", cacheDatas: _cache as NSDictionary)
        QZHMarketFindCompanyModel.searchParam = textStr.trimmingCharacters(in: .whitespaces)
        QZHMarketFindCompanyModel.searchParam = textStr.trimmingCharacters(in: .whitespaces)
        dismiss(animated: true, completion: nil)
    }
