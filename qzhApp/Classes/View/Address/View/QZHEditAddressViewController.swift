//
//  QZHEditAddressViewController.swift
//  qzhApp
//
...
class QZHEditAddressViewController: QZHBaseViewController {
    // 地址列表视图懒加载
    ...
    override func loadData() {
        self.addressStatus.getAddressInfo { (isSuccess) in
            self.txtName.text = self.addressStatus.addressInfoStatus[0].status.personName
            self.txtPhone.text = self.addressStatus.addressInfoStatus[0].status.phone
            self.txtArea.text = self.addressStatus.addressInfoStatus[0].status.areaInfo
            self.txtArea.restorationIdentifier = self.addressStatus.addressInfoStatus[0].status.area
            self.txtAddress.text = self.addressStatus.addressInfoStatus[0].status.detailedAddress
            if self.addressStatus.addressInfoStatus[0].status.isDefault == 1{
                self.defaultBtn.isOn = true
            }
        }
    }
}
...
// MARK: - 设置页面 UI 样式
extension QZHEditAddressViewController{
    ...
    func setupForm(){
        ...
        selBtn.setupViews(x: 650*PX, y: 0, width: 100*PX, height: 80*PX, bgColor: UIColor.clear)
...
extension QZHEditAddressViewController{
    ...
    func saveActions(){
        ...
        if QZHAddressModel.person == ""{
            ...
            self.addressStatus.editAddress { (isSuccess, result) in
