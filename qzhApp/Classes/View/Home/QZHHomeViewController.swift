...
class QZHHomeViewController: QZHBaseViewController {
    ...
    override func loadData(){
       ...
       statusList.loadHomeAd { (lbt, sqsc, cysq, qymh, jfyg, isSuccess) in
            if isSuccess{
                //设置轮播图
                var imgArray:[String] = []
...
extension QZHHomeViewController{
    ...
    func setupModuleView(){
        ...
        self.setupModule(selfView: icon1, x: 71*PX, bg: "ICON1", img: "ICON_1", title: "社区商城", action: #selector(goTosqsc(_:)))
        ...
        self.setupModule(selfView: icon3, x: 419*PX, bg: "ICON3", img: "ICON_3", title: "企业门户", action: #selector(EnterprisePortalPage))
        parentView.addSubview(icon3)
        
        let icon4:QZHUIView = QZHUIView()
        self.setupModule(selfView: icon4, x: 594*PX, bg: "ICON4", img: "ICON_4", title: "积分优购", action: #selector(goTosqsc(_:)))
        parentView.addSubview(icon4)
        
        tabbelView?.addSubview(parentView)
    }
    // 设置模块图标
    func setupModule(selfView:QZHUIView,x:CGFloat,bg:String,img:String,title:String,action:Selector){
        ...
        bgView.backgroundColor = UIColor(patternImage: UIImage(named:bg)!)
