...
extension HHGuidePageHUD {
    ...
    func addSkipButton(isHiddenSkipButton: Bool) -> Void {
        ...
        let skipButton = UIButton.init(frame: CGRect.init(x: HHScreenWidth * 0.8, y: HHScreenWidth * 0.1 + 24, width: 40, height: 25))
        skipButton.setTitle("跳过", for: .normal)
        skipButton.backgroundColor = UIColor.gray
        skipButton.setTitleColor(UIColor.white, for: .normal)
         skipButton.titleLabel?.font = UIFont.systemFont(ofSize: 20*PX)
    ...
    func addImages() -> Void {
        ...
        for i in 0..<imageArray.count {
            ...
            let idString = (imageArray[i] as NSString).substring(from: imageArray[i].getLength() - 3)
            if idString == "gif" {
            ...
            if i == imageArray.count - 1 {
                ...
                let startButton = UIButton.init(frame: CGRect.init(x: 150*PX, y: SCREEN_HEIGHT-170*PX, width: 450*PX, height: 100*PX))
                startButton.setTitle("立即体验", for: .normal)
                startButton.setTitleColor(myColor().blue007aff(), for: .normal)
    ...
    func addPageControl() -> Void {
        // 设置引导页上的页面控制器
        self.imagePageControl = UIPageControl.init(frame: CGRect.init(x: 0, y: SCREEN_HEIGHT-46*PX, width: SCREEN_WIDTH, height: 19*PX))
        self.imagePageControl?.currentPage = 0
        self.imagePageControl?.numberOfPages = self.imageArray?.count ?? 0
        self.imagePageControl?.pageIndicatorTintColor = myColor().GrayF1F2F6()
        self.imagePageControl?.currentPageIndicatorTintColor = myColor().blue007aff()
...
extension HHGuidePageHUD: UIScrollViewDelegate {
    ...
    func tickDown(){
        if guidePageView.contentOffset.x / guidePageView.frame.size.width == 2{
            self.skipButtonClick()
        }else{
            guidePageView.contentOffset.x = guidePageView.contentOffset.x + SCREEN_WIDTH
        }
        let page = guidePageView.contentOffset.x / guidePageView.frame.size.width
        self.imagePageControl?.currentPage = Int(page)
    }
